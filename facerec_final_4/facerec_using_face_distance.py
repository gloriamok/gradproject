...
def face_check_execute():
    ...
    if row != None:
        ...
        userId_dict = {"jooyoungmok":"person_1", "chaeyuncho":"person_2", "heesoolee":"person_3", "sangmilee":"person_4"}
        ...
        if (len(image_to_test_encoding) == 0) :
            ...
            py_serial = serial.Serial("/dev/ttyUSB0", 9600)
            print("arduino connection completed")
            time.sleep(3)
            py_serial.write(facerec_result.encode())
            time.sleep(3)
            py_serial.close()
